...
-- Solution
select stock_name, (select sum(price) from stocks as s2
                    group by stock_name, operation
                    having operation = "Sell"
                    and s1.stock_name = s2.stock_name) - (select sum(price) from stocks as s2
                                                          group by stock_name, operation
                                                          having operation = "Buy"
                                                          and s1.stock_name = s2.stock_name) as capital_gain_loss
from (select distinct stock_name from stocks) as s1
